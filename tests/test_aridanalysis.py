...
def test_logreg_model_inputs(log_df):
    with pytest.raises(AssertionError, match=errors.NO_VALID_FEATURES):
        aa.arid_logreg(log_df, response="Target", features="Age", type="binomial") # noqaE501
    with pytest.raises(AssertionError, match=errors.INVALID_DATAFRAME):
        aa.arid_logreg(17, response="Target", features=["Age", "Sex"], type="multinomial") # noqaE501
    with pytest.raises(AssertionError, match=errors.EMPTY_DATAFRAME):
        aa.arid_logreg(
            pd.DataFrame(), response="Target", features=["Age", "Sex"], type="binomial" # noqaE501
        )
    with pytest.raises(AssertionError, match=errors.RESPONSE_NOT_FOUND):
        aa.arid_logreg(
            df=log_df, response="targ", features=["Age", "Sex"], type="binomial" # noqaE501
        )
    with pytest.raises(AssertionError, match=errors.INVALID_TYPE_INPUT):
        aa.arid_logreg(
            log_df, response="Target", features=["Age", "Sex"], type="ordinal"
        )
...
def test_logreg_model_outputs(log_df):
    assert (
        round(
            (
                aa.arid_logreg(
                    df=log_df, response="Target", features=[], type="binomial"
                )[0].coef_
            )[0][0],
            3,
        )
    ...
    assert (
        round(
            (
                aa.arid_logreg(
                    log_df, response="Target", features=["Age"], type="binomial" # noqaE501
                )[0].coef_
            )[0][0],
            3,
        )
    ...
    assert (
        type(
            aa.arid_logreg(df=log_df, response="Target", features=[], type="binomial")[ # noqaE501
                1
            ]
        )
    ...
    assert (
        ...
        == statsmodels.discrete.discrete_model.MultinomialResultsWrapper
    )
...
def health_df():
    ...
    Create a basic test dataframe for linear regression tests
    """
    health_df = pd.read_csv("tests/toy_data/badhealth.csv").drop(columns=["Unnamed: 0"]) # noqaE501
...
def test_countreg_model_inputs(health_df):
    with pytest.raises(AssertionError, match="ERROR: INVALID LIST INTPUT PASSED"): # noqaE501
        aa.arid_countreg(
            health_df,
            response="numvisit",
            con_features="age",
    ...
    with pytest.raises(
        AssertionError,
        match="ERROR: INVALID RESPONSE DATATYPE FOR COUNT REGRESSION: MUST BE TYPE INT", # noqaE501
    ):
        aa.arid_countreg(
            data_frame=health_df,
            response="badh",
...
def test_countreg_model_outputs(health_df):
    ...
    # In scikit learn interactions do not change the number of coefficients,
    # this a weighted depening con correlation with other features
    assert (
        len(
            aa.arid_countreg(
                health_df,
    ...
    assert (
        str(
            type(
                aa.arid_countreg(
                    data_frame=health_df, response="numvisit", model="additive"
                )[0]
            )
        )
    ...
    assert (
        str(
            type(
                aa.arid_countreg(
                    data_frame=health_df, response="numvisit", model="interactive" # noqaE501
                )[1]
            )
        )
        == "<class 'statsmodels.genmod.generalized_linear_model.GLMResultsWrapper'>" # noqaE501
    )
